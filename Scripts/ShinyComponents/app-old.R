   ...
   tabPanel("Statistical Model",
    fluidRow(
      ...
      column(2,
         ...
         checkboxInput(inputId = "variance", label="Standardize Components by Variance"),
...
print.background.info <- function( background.id ){
  ...
  if(background.id == "Average"  | background.id == "Atlas" ){
    for(i in 1:length(variables) ){
      res <- sprintf("%s <tr><td>%s</td><td> %f</td><td> %f</td></tr>", res, variables[[i]]$name,
                     mean(variables[[i]]$value, na.rm=TRUE), sd(variables[[i]]$value, na.rm=TRUE) )
    }
  }else{
    subject.id = as.integer( substr( background.id, nchar(background.id)-3 , nchar(background.id) ) )
    for(i in 1:length(variables) ){

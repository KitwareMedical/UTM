...
option_list = list(
  ...
  make_option( "--ms", type="character", default="VBM-ms.Rdata",
    help="Morse-Smale components data file", metavar="character")
)
...
config$barycenters$euclidean =
  sprintf("./%s/barycenter-euclidean.Rdata", config$barycenterfolder)
config$barycenters$otdiscrete =
  sprintf("./%s/barycenter-ot-discrete.Rdata", config$barycenterfolder)
...
config$ms.file = sprintf("./%s/Images/%s", config$analysisfolder, config$ms )
...
try( setwd( config$workingfolder )
    ,
    silent=TRUE )
...
try({
  ...
  component.types = append(component.types, "PCA")
  feature.names = pcs$names
},
silent=TRUE
)
try({
  load( config$pcs.reg.file )
  component.types = append(component.types, "SpatCA")
},
silent=TRUE
)
try({
  load( config$ms.file )
  component.types = append(component.types, "Morse-Smale")
  component.types = append(component.types, "Morse-Smale-2")
  #build components from partition & variances
  ms.comps$components <- list()
  ms.comps$components2 <- list()
  for(k in 1:length(ms.comps$partitions)){
    n.part = max(ms.comps$partitions[[k]])
    ms.comps$components[[k]] <- matrix(0, nrow=n.part, ncol=length(ms.comps$variances[[k]]))
    ms.comps$components2[[k]] <- matrix(0, nrow=n.part, ncol=length(ms.comps$variances[[k]]))
    for(i in 1:n.part){
      ind = which(ms.comps$partitions[[k]] == i)
      l = sqrt(sum(ms.comps$variances[[k]][ind]))
      ms.comps$components2[[k]][i, ind] = sqrt(ms.comps$variances[[k]][ind])/l
      l = sqrt(length(ind))
      ms.comps$components[[k]][i, ind] = 1/l
    }
  }
  feature.names = ms.comps$names
},
silent=TRUE
)
print(feature.names)
...
try(
  load( config$barycenters$otdiscrete )
, silent=TRUE )
...
background.names <- c("Average")
atlas <- NULL
try({
  load(config$atlas)
  background.names <- append(background.names, "Atlas", after=0)
}
, silent=TRUE)
default.js.code ="
  ...
  tabPanel("Explore",
   fluidRow(
    column(3, align="left", offset = 0,
     bsCollapse(multiple=TRUE,
      ...
      bsCollapsePanel("Statistical Models",
       selectInput( inputId = "feature", label="Analysis:", choices=feature.names  ),
       ...
       selectInput( inputId = "component", label="Components:", choices=component.types),
       selectInput( inputId = "variable", label="Variable:", choices=var.names ),
       bsPopover("variable", "Variable Selection",
                 "Show correlations with respect to selection",
                 "right", options = list(container = "body")),
       ...
       bsCollapsePanel("Include Control Variables",
        checkboxGroupInput(inputId = "control", label="Control for:",
                           choices=var.names)
       ),
       br(),
       bsCollapse(
        bsCollapsePanel("Regression Models",
         ...
         bsCollapse(id = "model",
          bsCollapsePanel("Joint Linear",
           "Multiple linear regression on all components"),
           bsCollapsePanel("Elastic Net",
            "Elastic net regression on all components",
    ...
    column(9, align="left", style='padding:2px;',
      ...
      tabPanel("Slice View",
       fluidRow(
        column(3, align="center", offset = 0, style='padding:2px;',
         plotOutput(outputId = "axial", width = "300px",
                    click = "axial_click",
                    dblclick = "axial_dblclick", hover = "axial_hover" )
        ),
        column(3, align="center", offset = 0, style='padding:2px;',
         plotOutput(outputId = "coronal", width = "300px",
                    click = "coronal_click",
                    dblclick = "coronal_dblclick", hover = "coronal_hover")
        ),
        column(3, align="center", offset = 0, style='padding:2px;',
         plotOutput(outputId = "sagittal", width = "300px",
                    click = clickOpts(id="sagittal_click"),
        ...
        column(3, align="left",
         ...
         br(),
         textOutput( outputId = "text.value" ),
         hr(),
     ...
     hr(),
     bsCollapse(
      bsCollapsePanel("Statistical Model Output",
       fluidRow(
        column(5,
...
server <- function(input, output, session) {
  ...
  fitted.image <- reactive({
    ...
    im.dim = get.data.frame()$dim
    x = estimates$coef
    x[estimates$p.value > input$p.threshold] = 0
    image = array(t(comps) %*% x[1:nrow(comps)], dim=im.dim )
    p.image = array(0, dim=im.dim)
  ...
  observe.fitted <- observe({
    try({
    i.max = signif(get.max.color( fitted.image()$image ), 3)

    updateSliderInput(session, "threshold", max=i.max, step=i.max/100, value=5*i.max/100 )
    })
  })
  ...
  observe.fitted <- observe({
    cmax = signif( get.max.color( fitted.image()$image ), 3 )
  ...
  output$axial <- renderPlot({
    ...
    plot.slice( fitted.image()$image, fitted.image()$p.image, input$feature, input$max.color,
  ...
  output$coronal <- renderPlot({
    slices = c(input$sagittalslice,  input$coronalslice, input$axialslice)
    plot.slice( fitted.image()$image, fitted.image()$p.image,
                input$feature, input$max.color,
                input$transparency, input$threshold,
  ...
  output$sagittal <- renderPlot({
    ...
    plot.slice( fitted.image()$image, fitted.image()$p.image,
                input$feature, input$max.color,
                input$transparency, input$threshold, input$p.threshold,
                input$background,
  ...
  output$volume1 <- renderVtkWidget({
    ...
    widget = vtkWidget(data=list(image = im,
  ...
  #Info / Controls
  output$text.value <- renderText({
    sprintf( "Effect: %.01e",
       fitted.image()$image[input$sagittalslice, input$coronalslice, input$axialslice]
    )
  })

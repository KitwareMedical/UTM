...
basefolders <- list.dirs(".", recursive=FALSE, full.names=FALSE)
...
load.data <- function(folder){
  ...
  load( sprintf("./%s/%s", folder, config$variablesfile), env   )

  load( sprintf("./%s/%s", folder, config$correlationfile), env  )

  load( sprintf("./%s/%s", folder, config$barycenters$euclidean) )
  barycenter.euclidean <<- barycenter

  try(suppressWarnings( load( sprintf("./%s/%s", folder, config$barycenters$otdiscrete) ) ), silent=TRUE )
...
ui <- fluidPage(theme = shinytheme("lumen"),
  ...
  fluidRow(
      column(3,
         selectInput( inputId = "tissue", label="Tissue:", choices=basefolders ),
...
plot.slice <- function(variable, feature, maxcor, transparency, threshold, background, slice, axis, vslice){
...
server <- function(input, output, session) {
  ...
  observe.tissue <- observe({
    load.data( input$tissue )
  })
  ...
  get.max.cor <- function(variable){
    ...
    for(i in 1:length(features ) ){
      cmax = max( cmax, max(abs(features[[i]]) ) )
    }
    cmax
  ...
  output$downloadAllSlices <- downloadHandler(
    ...
    content = function(file) {
      ...
      for(variable in names(cor.res) ){
        cmax <- get.max.cor( variable )
